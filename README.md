...
| **Capability Bounding** | Keychain-Auth (Subsystem) | Static, long-lived, over-privileged local credentials. OS keychain access is verified against the calling process's cryptographic hash, so only authorized binaries can resolve a credential, even from the same machine. |
...
|---|---|
| [Environments & Team Workspaces](https://agentsecrets.theseventeen.co/docs) | Dev/staging/prod isolation, zero-knowledge team sync, onboarding without sharing plaintext |
| [Agent Identity & Policies](https://agentsecrets.theseventeen.co/docs) | Cryptographic agent tokens, per-agent allow/deny lists, secret-level domain and method rules, interactive approval flow |
| [Auth Injection Styles](https://agentsecrets.theseventeen.co/docs) | All six injection modes: bearer, header, query, basic, JSON body, and form field |
| [Zero-Trust Proxy Pipeline](https://agentsecrets.theseventeen.co/docs) | The full request pipeline, including allowlisting, capability checks, SSRF protection, and response redaction |
| [Encryption Model](https://agentsecrets.theseventeen.co/docs) | X25519 key exchange, AES-256-GCM, Argon2id, OS keychain storage, and SHA-256 audit chaining |
| [Build on AgentSecrets](https://agentsecrets.theseventeen.co/docs/sdk) | Python SDK, zero-knowledge MCP template, and the JS/TS SDK (coming soon) |
| [Full Command Reference](https://agentsecrets.theseventeen.co/docs) | Every subcommand across account, workspace, project, environment, secrets, proxy, and audit |

## Roadmap · Security · Contributing

- **Roadmap:** see the [engineering blog](https://engineering.theseventeen.co/series/building-agentsecrets) for what's shipping next.
